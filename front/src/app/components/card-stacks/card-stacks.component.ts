...
export class CardStacksComponent {
  ...
  displayedCredentialStackEntries = toSignal(this.displayedCredentials$.pipe(
    map(credentials => {
      let credentialStacks: any = {};
      for (const credential of credentials) {
        const stackName = credential.decryptedData?.title.charAt(0).toLocaleUpperCase() || 'Uncategorized';
